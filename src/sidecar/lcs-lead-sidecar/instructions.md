...
These skills are triggered automatically at specific points — no user command needed:

| Skill | Trigger Point | Skill Name |
|-------|--------------|------------|
| Pre-flight | First activation (no existing state) | `bmad-lcs-preflight` |
| Risk Map | Immediately after Audit completes | `bmad-lcs-risk-map` |
| Framework Profile | During Pre-flight (step 2) or Audit | `bmad-lcs-framework-profile` |
| Dependency Update | Immediately after Safety Nets completes | `bmad-lcs-dependency-update` |
| Guard | After Deep Testing completes (ongoing) | `bmad-lcs-guard` |
